...
export const PlayerContext = createContext({} as ReturnType<typeof usePlayerLogic>);
...
export function usePlayerLogic() {
    ...
    useEffect(() => {
        if (!getUserQueue || !playerReady) {
            return;
        }
        getUserQueue();
    }, [getUserQueue, playbackData, playerReady]);
        ...
        async function authenticate() {
            ...
            if (state === localStorage.getItem('spotify_auth_state')) {
                try {
                    ...
                    refreshIntervalRef.current = setInterval(refreshAccessToken, expiresIn);
                    router.push('/player');
                } catch (error) {
    ...
    useEffect(() => {
        const { duration: durationMs, position: positionMs, track_window: trackInfo } = playbackData ?? {};
        ...
        intervalRef.current = setInterval(() => {
            ...
            setProgressPercent((currentPosition.current / currentTrackDuration) * 100);
        }, 1000);
